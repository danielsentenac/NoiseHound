"""
plot_sr_mar_tx_timeseries.py — SAT_SR_MAR_TX_SET + glitch properties, full O4.

3 panels sharing the same x-axis (full O4, 2023-2026):
  Panel 1:  SAT_SR_MAR_TX_SET
  Panel 2:  Glitch median SNR (daily bins)
  Panel 3:  Glitch Q-factor — median + fraction above Q_min (daily bins)

Usage:
    python scripts/plot_sr_mar_tx_timeseries.py \\
        --sr-csv    outputs/sr_o4_merged.csv \\
        --glitches  data/full_25min_glitches_ER16-O4b.csv \\
        --output    usecases/25-minute-glitch/sr_mar_tx_timeseries.png
"""
...
def detect_steps(tx: np.ndarray, gps: np.ndarray,
    ...
    """Detect persistent level changes in TX (sliding-window median comparison)."""
    steps = []
    half = 6
...
def main():
    ...
    ap.add_argument("--sr-csv",   default="outputs/sr_o4_merged.csv")
    ap.add_argument("--glitches", default="data/full_25min_glitches_ER16-O4b.csv")
    ap.add_argument("--output",   default="usecases/25-minute-glitch/sr_mar_tx_timeseries.png")
    args = ap.parse_args()
    ...
    fig.suptitle("SAT_SR_MAR_TX_SET + glitch properties — full O4 (2023–2026)",
                 fontsize=11, y=0.995)

    # ── Panel 0: MAR TX ─────────────────────────────────────────────────────
    ax0 = axes[0]
    ax0.plot(sr_dt, tx, color="steelblue", lw=0.8, label="SAT_SR_MAR_TX_SET")
    ax0.axhline(-757, color="darkgreen", lw=0.9, ls="--", alpha=0.7,
                label="LN3_ALIGNED level (~−757)")
    ax0.axvline(last_gl_dt, color="crimson", lw=1.2, ls="--",
                label=f"Last glitch {str(last_gl_dt)[:10]}")
    add_lock_spans(ax0)
    # Step markers on panel 0
    for dt, dv in zip(step_dts, step_deltas):
        color = "darkorange" if dv > 0 else "purple"
        ax0.axvline(dt, color=color, lw=0.7, ls=":", alpha=0.8)
    ax0.set_ylabel("TX setpoint [arb]", fontsize=9)
    ax0.tick_params(labelsize=8)
    # Custom legend entries for steps
    from matplotlib.lines import Line2D
    from matplotlib.patches import Patch
    step_handles = [
        Line2D([0], [0], color="darkorange", lw=0.7, ls=":", label="TX step up"),
        Line2D([0], [0], color="purple",     lw=0.7, ls=":", label="TX step down"),
    ...
    add_lock_spans(ax1)
    for dt, dv in zip(step_dts, step_deltas):
        ax1.axvline(dt, color="darkorange" if dv > 0 else "purple",
                    lw=0.7, ls=":", alpha=0.8)
    ...
    add_lock_spans(ax2)
    for dt, dv in zip(step_dts, step_deltas):
        ax2.axvline(dt, color="darkorange" if dv > 0 else "purple",
                    lw=0.7, ls=":", alpha=0.8)
